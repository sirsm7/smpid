...
if (window.supabase) {
    ...
    alert("Supabase gagal dimuatkan. Sila refresh.");
}
...
const state = {
    userKod: null,
    isAdmin: false,
    dashboardData: [], // Cache untuk dashboard
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Semak jika ada sesi tersimpan
    ...
    // 3. Setup Input Listener (Uppercase automatik)
...
function navigateTo(viewId) {
    // 1. Sembunyikan semua seksyen
    document.querySelectorAll('.view-section').forEach(el => {
        el.classList.remove('active');
        setTimeout(() => el.classList.add('hidden'), 100); // Micro-delay for smooth reflow
    });

    // 2. Paparkan seksyen sasaran
    ...
    // 3. Logik khusus mengikut View
    updateHeader(viewId);
    
    if (viewId === 'menu') initMenu();
    if (viewId === 'profile') loadProfileData();
    if (viewId === 'dashboard') {
        if (!state.isAdmin) { navigateTo('login'); return; } // Guard
        loadDashboardData();
    }
    if (viewId === 'email') {
        if (!state.isAdmin) { navigateTo('login'); return; } // Guard
        // Jika data dashboard dah ada, guna je untuk email list
...
async function prosesLogin() {
    ...
    if (kod === "M030") {
        ...
        state.isAdmin = true;
        state.userKod = null; // Admin tak ada kod sekolah spesifik
        
        Swal.fire({
            icon: 'success', title: 'Log Masuk Admin', timer: 1000, showConfirmButton: false
...
function logout() {
    Swal.fire({
        ...
        if (res.isConfirmed) {
            ...
            state.isAdmin = false;
            state.userKod = null;
            state.dashboardData = []; // Clear cache
            document.getElementById('inputKodSekolah').value = ""; // Reset input
            navigateTo('login');
        }
    });
}
...
// --- MODULE 2: MENU ---
function initMenu() {
    // Tapis kad menu berdasarkan role
    if (state.isAdmin) {
...
async function loadProfileData() {
    ...
    // Jika admin masuk profil, dia mesti view sekolah tertentu. 
    // (Nota: Dalam SPA ini, admin biasanya akses profil melalui Dashboard. 
    // Kod ini menganggap session 'smpid_user_kod' diset bila klik dari dashboard)
    ...
    if (state.isAdmin) {
        btnPadam.classList.remove('hidden');
    } else {
        btnPadam.classList.add('hidden');
    }
...
async function loadDashboardData() {
    // Jika data dah ada (cache), guna je. Kecuali nak force refresh.
...
function renderActionBtns(nama, tel, teleId) {
    ...
    if (teleId) {
        html += `<span class="badge bg-primary bg-opacity-10 text-primary border border-primary"><i class="fas fa-check-circle"></i> OK</span>`;
    } 
    const linkRaw = generateWhatsAppLink(nama, tel, true);
    const linkTemplate = generateWhatsAppLink(nama, tel, false);

    if (linkRaw) {
        html += `<a href="${linkRaw}" target="_blank" onclick="event.stopPropagation()" class="btn btn-sm btn-light border text-secondary" title="Mesej"><i class="fas fa-comment"></i></a>`;
    }
    if (!teleId && linkTemplate) {
         html += `<a href="${linkTemplate}" target="_blank" onclick="event.stopPropagation()" class="btn btn-sm btn-outline-success" title="Ingatkan"><i class="fab fa-whatsapp"></i></a>`;
    }
    return html + '</div>';
}
...
function mulaTindakanPantas() {
    ...
    list.forEach(i => {
        if (cleanPhone(i.no_telefon_gpict) && !i.telegram_id_gpict) {
            state.reminderQueue.push({role:'GPICT', ...i, targetName: i.nama_gpict, targetTel: i.no_telefon_gpict});
        }
        if (cleanPhone(i.no_telefon_admin_delima) && !i.telegram_id_admin) {
            state.reminderQueue.push({role:'Admin', ...i, targetName: i.nama_admin_delima, targetTel: i.no_telefon_admin_delima});
        }
    });
...
function renderQueueCard() {
    if (state.queueIndex >= state.reminderQueue.length) {
        toggleQueueModal(false); Swal.fire('Selesai', 'Semua peringatan disemak.', 'success'); return;
    }
